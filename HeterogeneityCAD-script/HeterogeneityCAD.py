...
################
#below are for the commandline version of this module
################

class HeterogeneityCADlogic:
  def __init__(self, volumenode, labmap):
    self.FeatureVectors = []
    self.volumenode = volumenode
    self.lablemap = labmap
    self.featurekeys = ["Node","Voxel Count", "Gray Levels", "Energy", "Entropy" , "Minimum Intensity", "Maximum Intensity", "Mean Intensity", "Median Intensity", "Range", "Mean Deviation", "Root Mean Square",  "Standard Deviation", "Skewness", "Kurtosis", "Variance", "Uniformity",
	       "Volume mm^3", "Volume cc", "Surface Area mm^2", "Surface:Volume Ratio", "Compactness 1", "Compactness 2", "Maximum 3D Diameter", "Spherical Disproportion", "Sphericity",
	       "Autocorrelation", "Cluster Prominence", "Cluster Shade", "Cluster Tendency", "Contrast", "Correlation", "Difference Entropy", "Dissimilarity", "Energy (GLCM)", "Entropy(GLCM)", "Homogeneity 1", "Homogeneity 2", "IMC1", "IDMN", "IDN", "Inverse Variance", "Maximum Probability", "Sum Average", "Sum Entropy", "Sum Variance", "Variance (GLCM)",
	       "SRE", "LRE", "GLN", "RLN", "RP", "LGLRE", "HGLRE", "SRLGLE", "SRHGLE", "LRLGLE", "LRHGLE"]
    nodeLogic = FeatureExtractionLogicCLI(self.volumenode, self.lablemap, self.featurekeys)
    self.FeatureVectors.append(nodeLogic.getFeatureVector())
...
class FeatureExtractionLogicCLI:
...
if __name__=="__main__": 
  ...
  for num,i in enumerate(index):
    try:
      if num==0:
	a = re.search(r"(.*)/(.*).nrrd",sys.argv[i+2],re.S)
	b = re.search(r"(.*)/(.*).nrrd",sys.argv[i+number+2],re.S)
	volume = a.group(2)
	label = b.group(2)
	slicer.util.loadVolume(sys.argv[i+2])
	n = getNode(volume)
	slicer.util.loadVolume(sys.argv[i+2+number])
	m = getNode(label)
	fakeobject = HeterogeneityCADlogic(n,m)
	fakeobject.populateStatistics(fakeobject.FeatureVectors)  
	fakeobject.saveStatistics(sys.argv[i+2*number+2])
      elif num!=0:
	a = re.search(r"(.*)/(.*).nrrd",sys.argv[i+2],re.S)
	b = re.search(r"(.*)/(.*).nrrd",sys.argv[i+number+2],re.S)
	volume = a.group(2)
	label = b.group(2)
	sn = n.GetStorageNode()
	sn.SetFileName(sys.argv[i+2])
	sn.ReadData(n)
	sm = m.GetStorageNode()
	sm.SetFileName(sys.argv[i+number+2])
	sm.ReadData(m)
	fakeobject = HeterogeneityCADlogic(n,m)
	fakeobject.populateStatistics(fakeobject.FeatureVectors)  
	fakeobject.saveStatistics(sys.argv[i+2*number+2])
	display = sys.argv[i+2] + " is done!"
	print(display)
    except:
      print("getting trouble when dealing with")
